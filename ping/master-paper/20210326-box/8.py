...
df1 = df[['病例组第1天SOFA评分', '病例组3天SOFA评分']]
# df1 = df1.dropna()
print(df1["病例组第1天SOFA评分"].dropna())
print(df1["病例组3天SOFA评分"].dropna())
ax1.boxplot([df1["病例组第1天SOFA评分"].dropna(), df1["病例组3天SOFA评分"].dropna()], notch=False, sym='o', vert=True)
plt.xticks([x+1 for x in range(2)], ['第1天', '第3天'], fontsize=20)
plt.yticks(fontsize=20)
plt.ylabel("病例组SOFA评分", fontsize=16)
# 绘制比较线
x1, x2 = 1, 2
y, h, col = 15, 1, 'k'
ax1.set_ylim(0, 18)
