...
df1 = df[["第1天与第3天单核细胞PD-L1差值（%）.1", "第1天与第3天降钙素原差值（ng/ml）"]]
df1 = df1.dropna()
X = df1["第1天与第3天单核细胞PD-L1差值（%）.1"].values.reshape(-1, 1)
y = df1["第1天与第3天降钙素原差值（ng/ml）"].values.reshape(-1, 1)
...
ax1.text(x=50, y=90, s='$r=0.764$', fontsize=20)
ax1.text(x=50, y=110, s='$^{*}p=0.006$', fontsize=20)
